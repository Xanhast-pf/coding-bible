...
import { createFinding, getReferences, insertBeforeNodeEdit, nodesOfKind, } from "../../utils.mjs";
const isTypeOnlyUsage = (identifier) => {
    let current = identifier.parent;
    while (!ts.isStatement(current) &&
...
export const ts003TypeOnlyImportsDetector = {
    ...
    analyze: (context) => {
        const findings = [];
        for (const statement of nodesOfKind(context, ts.SyntaxKind.ImportDeclaration)) {
            ...
            for (const specifier of importClause.namedBindings.elements) {
                ...
                const usages = getReferences(context, specifier.name).filter((identifier) => identifier !== specifier.name &&
                    identifier !== specifier.propertyName);
                if (!usages.length || !usages.every(isTypeOnlyUsage)) {
                    continue;
                }
                findings.push(createFinding(context, specifier, {
...
export const ts003Detectors = [
    ts003TypeOnlyImportsDetector,
];
