...
import { createFinding, getSymbol, nodesOfKind, unwrapExpression, } from "../../utils.mjs";
const isExternalDataRead = (expression) => {
...
const isUnknownAssertion = (node) => node.type.kind === ts.SyntaxKind.UnknownKeyword;
...
const getTaintedSymbol = (context, expression, taintedSymbols) => {
    const candidate = unwrapExpression(expression);
...
const collectTaintedSymbols = (context) => {
    const taintedSymbols = new Set();
    const declarations = nodesOfKind(context, ts.SyntaxKind.VariableDeclaration);
    let changed = true;
    while (changed) {
        changed = false;
...
export const ts004UntrustedAssertionsDetector = {
    ...
    analyze: (context) => {
        ...
        for (const node of assertions) {
            ...
            findings.push(createFinding(context, node, {
                ...
                ruleId: "TS-004",
                suggestion: "Keep the value `unknown`, validate it at the boundary, then return the validated domain type.",
            }));
        }
        return findings;
    },
};
export const ts004Detectors = [
    ts004UntrustedAssertionsDetector,
];
