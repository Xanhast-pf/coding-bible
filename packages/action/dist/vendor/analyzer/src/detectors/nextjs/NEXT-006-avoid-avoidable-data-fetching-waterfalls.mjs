...
const usesSymbol = (context, node, symbol) => {
    let used = false;
        ...
        const inspectStatements = (statements) => {
            for (let index = 0; index < statements.length - 1; index += 1) {
                ...
                if (!dataCallName.test(firstName) || !dataCallName.test(secondName))
                    continue;
                const symbol = context.checker.getSymbolAtLocation(first.declaration.name);
                if (symbol && usesSymbol(context, second.call, symbol))
                    continue;
                findings.push(createFinding(context, second.call, {
                    detectorId: "next_006-independent-sequential-data-awaits",
                    ruleId: "NEXT-006",
                    message: "Two adjacent data-like requests are awaited sequentially even though the second does not depend on the first result.",
                    suggestion: "Start independent requests together, for example with Promise.all, when their ordering is not required.",
                }));
            }
        };
        const visit = (node) => {
