...
const observableSetCall = (statement) => {
...
const inspectStatementList = (context, statements, findings) => {
    for (let index = 1; index < statements.length; index += 1) {
        ...
        findings.push(createFinding(context, current.statement, {
...
export const legend004BatchSiblingUpdatesDetector = {
    dependencyScope: "source-file",
    id: "legend-batch-sibling-updates",
    profile: {
        confidence: "strong",
        contextNote: "Separate updates can be intentional when observers must see intermediate states or the writes are separate logical events. Confirm they form one state transition before batching.",
        impact: "medium",
    },
    ruleId: "LEGEND-004",
    analyze: (context) => {
