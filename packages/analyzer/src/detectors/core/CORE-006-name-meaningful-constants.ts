...
const detector: Detector = {
  ...
  analyze: (context) =>
    nodesOfKind<ts.BinaryExpression>(context, ts.SyntaxKind.BinaryExpression)
      .filter((node) => {
        if (!compareKinds.has(node.operatorToken.kind)) return false;
        const left = node.left.getText(context.sourceFile);
        if (!policyName.test(left)) return false;
        if (!ts.isNumericLiteral(node.right)) return false;
        const value = Number(node.right.text);
        return Number.isFinite(value) && value !== 0 && value !== 1;
      })
      .map((node) =>
        createFinding(context, node.right, {
          detectorId: "core_006-policy-magic-number",
          ruleId: "CORE-006",
          message:
            "A policy-like comparison uses an unexplained numeric threshold.",
          suggestion:
            "Name the threshold so its policy meaning is explicit and easy to change.",
        }),
      ),
};
