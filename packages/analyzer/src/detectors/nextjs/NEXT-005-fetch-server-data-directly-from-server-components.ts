...
const detector = createSourceEvidenceDetector({
  ...
  message:
    "Fetch server data directly from Server Components evidence was detected in this source.",
  suggestion:
    "Apply NEXT-005: Fetch server data directly from Server Components.",
  find: (context) =>
    conjunctiveEvidence(
      context.source,
      [
