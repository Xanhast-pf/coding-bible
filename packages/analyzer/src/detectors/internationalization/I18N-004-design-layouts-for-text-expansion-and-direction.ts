...
const directionalProperty =
  /\b(?:margin-left|margin-right|padding-left|padding-right)\s*:/m;
...
const detector = createSourceEvidenceDetector({
  ...
  message:
    "A single CSS rule combines directional spacing, fixed width, and nowrap, which is brittle for RTL and text expansion.",
  suggestion:
    "Prefer logical properties and a layout that can grow or wrap translated text.",
  find: (context) => {
    const evidence = [];
    for (const match of context.source.matchAll(blockPattern)) {
      const body = match[2] ?? "";
      if (
        !directionalProperty.test(body) ||
