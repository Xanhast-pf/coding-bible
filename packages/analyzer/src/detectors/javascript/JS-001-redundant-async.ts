...
const isAsyncFunction = (node: ts.Node): node is AsyncFunction =>
  Boolean(
...
const hasAwaitSemantics = (node: AsyncFunction) => {
  if (
    (ts.isFunctionDeclaration(node) ||
      ts.isFunctionExpression(node) ||
      ts.isMethodDeclaration(node)) &&
...
const hasThrow = (node: AsyncFunction) => {
  let found = false;
  visitFunctionBody(node, (candidate) => {
...
const getReturnedExpressions = (
  ...
  const returned: (ts.Expression | null)[] = [];
  visitFunctionBody(node, (candidate) => {
...
const isPromiseTypeName = (name: ts.EntityName): boolean => {
...
const isPromiseTypeNode = (
  ...
  if (ts.isUnionTypeNode(type) || ts.isIntersectionTypeNode(type)) {
    return type.types.some((member) =>
      isPromiseTypeNode(context, member, seen),
    );
  }
...
const typeNodeHasPromiseReturningCall = (
  context: DetectorContext,
  type: ts.TypeNode,
  seen = new Set<ts.Symbol>(),
): boolean => {
  ...
  if (ts.isUnionTypeNode(type) || ts.isIntersectionTypeNode(type)) {
    return type.types.some((member) =>
      typeNodeHasPromiseReturningCall(context, member, seen),
    );
  }
  ...
  if (ts.isTypeLiteralNode(type)) {
    return type.members.some(
      (member) =>
        ...
        Boolean(member.type) &&
        isPromiseTypeNode(context, member.type as ts.TypeNode),
    );
  }
...
const hasExplicitPromiseReturnType = (
  context: DetectorContext,
  node: AsyncFunction,
): boolean => Boolean(node.type && isPromiseTypeNode(context, node.type));

const hasSyntacticPromiseContext = (
  context: DetectorContext,
  node: AsyncFunction,
): boolean => {
...
const isKnownPromiseFactory = (expression: ts.Expression): boolean => {
  ...
  return (
    ...
    ["all", "allSettled", "any", "race", "reject", "resolve", "try"].includes(
      callee.name.text,
    )
  );
};

const isDefinitelySynchronousSyntax = (expression: ts.Expression): boolean => {
  const candidate = unwrapExpression(expression);
  ...
  if (ts.isBinaryExpression(candidate)) {
    return (
      isDefinitelySynchronousSyntax(candidate.left) &&
  ...
  if (ts.isConditionalExpression(candidate)) {
    return (
      isDefinitelySynchronousSyntax(candidate.whenTrue) &&
...
const contextualReturnIsPromiseLike = (
  context: DetectorContext,
  node: AsyncFunction,
) => {
  if (!ts.isArrowFunction(node) && !ts.isFunctionExpression(node)) {
...
const hasIntentionalPromiseReturn = (
  context: DetectorContext,
  node: AsyncFunction,
  returned: readonly (ts.Expression | null)[],
) => {
  if (
    hasExplicitPromiseReturnType(context, node) ||
...
const getWrappedExpression = (node: AsyncFunction): ts.Node => {
...
const isPassedAsCallbackArgument = (node: AsyncFunction): boolean => {
  if (!ts.isArrowFunction(node) && !ts.isFunctionExpression(node)) {
    return false;
  }
  ...
  return (
    ...
    Boolean(current.parent.arguments?.includes(current as ts.Expression))
  );
};

const isDefaultCallbackContract = (node: AsyncFunction): boolean => {
  if (!ts.isArrowFunction(node) && !ts.isFunctionExpression(node)) {
    return false;
  }
  ...
  const current = getWrappedExpression(node);
  return (
    ts.isParameter(current.parent) && current.parent.initializer === current
  );
};

const isClearlyRedundantAsync = (
  context: DetectorContext,
  node: AsyncFunction,
) => {
  if (
  ...
  return returned.every(
    (expression) => expression && isDefinitelySynchronousSyntax(expression),
  );
};

export const js001RedundantAsyncDetector: Detector = {
    ...
    visit(context.sourceFile, (node) => {
      ...
      findings.push(
        createFinding(context, node, {
          ...
          message:
            "This function is marked async but has no asynchronous work or Promise-producing return path.",
          ruleId: "JS-001",
          suggestion:
            "Remove `async`, or keep it only when the function intentionally exposes a Promise-returning contract.",
        }),
      );
    });
...
export const js001Detectors = [
  js001RedundantAsyncDetector,
