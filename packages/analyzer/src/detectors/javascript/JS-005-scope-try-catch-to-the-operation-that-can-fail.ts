...
const detector = createSourceEvidenceDetector({
  ...
  profile: {
    ...
    impact: "medium",
    contextNote:
      "This detector uses conservative static evidence for JS-005; review surrounding intent before changing behavior.",
  },
  message:
    "Scope try/catch to the operation that can fail evidence was detected in this source.",
  suggestion: "Apply JS-005: Scope try/catch to the operation that can fail.",
  find: (context) =>
    conjunctiveEvidence(context.source, [
