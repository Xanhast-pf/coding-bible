...
const detector = createSourceEvidenceDetector({
  ...
  message:
    "Respect reduced motion preferences evidence was detected in this source.",
  suggestion: "Apply A11Y-006: Respect reduced motion preferences.",
  find: (context) =>
    conjunctiveEvidence(
      context.source,
      [/(?:transition|animation|scroll-behavior)\s*:/m, /(?:smooth|\d+m?s)/m],
      [/prefers-reduced-motion/m],
    ),
});
