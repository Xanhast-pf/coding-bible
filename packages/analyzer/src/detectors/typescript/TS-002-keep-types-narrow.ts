...
const detector = createSourceEvidenceDetector({
  ...
  profile: {
    ...
    impact: "high",
    contextNote:
      "This detector uses conservative static evidence for TS-002; review surrounding intent before changing behavior.",
  },
  message: "Keep types narrow evidence was detected in this source.",
  suggestion: "Apply TS-002: Keep types narrow.",
  find: (context) =>
    conjunctiveEvidence(context.source, [/\btype\s+\w+\s*=\s*string\s*;/m], []),
});
