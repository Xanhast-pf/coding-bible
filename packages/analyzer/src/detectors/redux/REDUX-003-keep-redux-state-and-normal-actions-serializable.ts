...
const detector = createSourceEvidenceDetector({
  ...
  message:
    "Keep Redux state and normal actions serializable evidence was detected in this source.",
  suggestion:
    "Apply REDUX-003: Keep Redux state and normal actions serializable.",
  find: (context) =>
    conjunctiveEvidence(
      context.source,
      [/\bdispatch\s*\([\s\S]*new\s+Date\s*\(\s*\)/m],
      [/\.toISOString\s*\(/m],
    ),
});
