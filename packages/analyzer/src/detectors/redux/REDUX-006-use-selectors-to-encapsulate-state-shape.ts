...
const detector = createSourceEvidenceDetector({
  ...
  profile: {
    ...
    impact: "medium",
    contextNote:
      "This detector uses conservative static evidence for REDUX-006; review surrounding intent before changing behavior.",
  },
  message:
    "Use selectors to encapsulate state shape evidence was detected in this source.",
  suggestion: "Apply REDUX-006: Use selectors to encapsulate state shape.",
  find: (context) =>
    conjunctiveEvidence(
      context.source,
      [/\buseSelector\s*\(\s*\(?\s*state\s*\)?\s*=>\s*state\.\w+\.\w+\.\w+/m],
      [],
    ),
});
