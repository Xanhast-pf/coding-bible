import type { AnalyzerTextEdit } from "./types.ts";

const contextLines = 3;

export const normalizeAnalyzerPatchPath = (filePath: string) =>
  filePath.replaceAll("\\", "/");

export const applyAnalyzerTextEdits = (
  source: string,
...
const lineIndexAt = (starts: readonly number[], offset: number) => {
  let low = 0;
...
const groupEdits = (source: string, edits: readonly AnalyzerTextEdit[]) => {
...
export const prepareAnalyzerTextEdits = (
  source: string,
  edits: readonly AnalyzerTextEdit[],
  filePath: string,
): readonly AnalyzerTextEdit[] => {
  const deduped = new Map<string, AnalyzerTextEdit>();
  ...
  const prepared = [...deduped.values()].sort(
...
export const createAnalyzerFilePatch = (
  filePath: string,
  source: string,
  edits: readonly AnalyzerTextEdit[],
) => {
