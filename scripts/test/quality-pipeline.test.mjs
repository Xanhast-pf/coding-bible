...
test("local check heals deterministic drift before strict verification", () => {
  ...
  assert.equal(
    scripts.generate,
    "pnpm registries:generate && pnpm automation:matrix:generate && pnpm rulebook:schema:generate && pnpm agent:generate && pnpm analyzer:build && pnpm action:build",
  );
});
...
test("strict verification is ordered and does not call mutating generators", () => {
  ...
  assert.equal(
    ...
    "pnpm registries:check && pnpm automation:matrix:check && pnpm rulebook:schema:check && pnpm agent:check && pnpm analyzer:check && pnpm action:check",
  );
